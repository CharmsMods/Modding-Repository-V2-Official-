...
/**
 * Filter flag indicating if the user ONLY wants to export formally modified assets.
 * @type {boolean}
 */
window.exportOnlyModified = true;
...
async function initializeGallery() {
    try {
        ...
        if (!exportOptionsPopup || !closeExportPopupButton || !exportClientButton || !exportBrowserButton) {
            ...
            // Event listeners for Client/Browser export are handled entirely by export-options-popup.js now
        }
        ...
        try {
            if (!window.ASSET_DB) throw new Error('ASSET_DB not found! Make sure asset-db.js is loaded.');

            for (const [folder, filename] of window.ASSET_DB.png || []) {
                const mediaPath = `./mod-assets/png/${filename}`;
                allAssets.push({
                    id: `png_${folder}_${filename}`,
                    folder, filename, type: 'png', mediaPath,
                    originalImageBlob: null, modifiedImageBlob: null, newImageBlob: null,
                    isModified: false, isNew: false, excluded: false
                });
            }
            window.updateConsoleLog('PNG asset list loaded.');

            for (const [folder, filename] of window.ASSET_DB.jpg || []) {
                const mediaPath = `./mod-assets/jpg/${filename}`;
                allAssets.push({
                    id: `jpg_${folder}_${filename}`,
                    folder, filename, type: 'jpg', mediaPath,
                    originalImageBlob: null, modifiedImageBlob: null, newImageBlob: null,
                    isModified: false, isNew: false, excluded: false
                });
            }
            ...
            for (const [folder, filename] of window.ASSET_DB.mp3 || []) {
                const mediaPath = `./mod-assets/mp3/${filename}`;
                allAssets.push({
                    id: `mp3_${folder}_${filename}`,
                    folder, filename, type: 'mp3', mediaPath,
                    originalImageBlob: null, modifiedImageBlob: null, newImageBlob: null,
                    isModified: false, isNew: false, excluded: false
                });
            }
            ...
            console.error('Error loading assets from ASSET_DB:', error);
            window.updateConsoleLog(`[ERROR] Failed to load asset lists: ${error.message}`);
        }
...
async function loadAllAssetsIntoMemory() {
    ...
    // Only load non-excluded image assets into memory (and check modified status if enabled)
    const imageAssets = allAssets.filter(asset => {
        if (asset.type !== 'png' && asset.type !== 'jpg') return false;
        if (asset.excluded) return false;
        if (window.exportOnlyModified) {
            // Unmodified? Skip loading it into memory.
            if (!asset.isModified && !asset.isNew) return false;
        }
        return true;
    });
    ...
    const excludedCount = allAssets.filter(asset => {
        if (asset.type !== 'png' && asset.type !== 'jpg') return false;
        if (asset.excluded) return true;
        if (window.exportOnlyModified && !asset.isModified && !asset.isNew) return true;
        return false;
    }).length;
    ...
    // Process in smaller batches to avoid freezing the UI thread with concurrent requests
    const BATCH_SIZE = 20;
    for (let i = 0; i < imageAssets.length; i += BATCH_SIZE) {
            ...
            if (!asset.originalImageBlob) {
                try {
                    ...
                    if (errorMessage.includes('NetworkError') || errorMessage.includes('Failed to fetch')) {
                        errorMessage += " (Likely browser security blocking file:// fetching. Must use local server for ZIPs)";
                    }
                    window.updateConsoleLog(`[ERROR] Failed to load blob for: ${asset.filename} - ${errorMessage}`);
                }
            } else {
        ...
        // Yield to browser rendering heavily to stop UI lockups
...
window.loadingOverlayTimeoutId = null; // Global tracker for overlay timeouts

window.showLoadingOverlay = (message) => {
    // Prevent any queued hide commands from prematurely closing this new overlay
...
let isZipping = false; // Prevents multiple concurrent ZIP downloads
...
async function initiateZipDownload(exportType) {
    if (isZipping) {
        console.warn('ZIP download is already in progress. Ignoring request.');
        return;
    }
    isZipping = true;
    ...
    // Filter out excluded (or unmodified) assets first
    const filteredAssets = window.allAssets.filter(asset => {
        if (asset.excluded) return false;
        if (window.exportOnlyModified && !asset.isModified && !asset.isNew) return false;
        return true;
    });
    ...
    if (excludedCount > 0) {
        const excludedAssets = window.allAssets.filter(asset => {
            if (asset.excluded) return true;
            if (window.exportOnlyModified && !asset.isModified && !asset.isNew) return true;
            return false;
        });
    ...
    if (!assetsLoadedIntoMemory) {
        ...
        // Wait 1 second before generating zip just to safely clear out the previous loading messages
    ...
    // Process the filtered assets (non-excluded) in batches to avoid locking the UI
    const BATCH_SIZE = 20;
    for (let i = 0; i < filteredAssets.length; i += BATCH_SIZE) {
        ...
        await Promise.all(batch.map(async (asset) => {
            ...
            try {
                ...
                window.updateConsoleLog(`[ERROR] Error processing: ${filename} - ${errorMessage}`);
    ...
    try {
        const content = await zip.generateAsync({
            type: "blob",
            compression: "STORE", // Fast mode: Do not deflate. JPG/PNG/MP3 are heavily naturally compressed natively!
            // DEFLATE is omitted because it causes massive CPU UI locking for <1% file saving.
        ...
        isZipping = false; // Release the lock
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (exportModeSelect) {
        exportModeSelect.addEventListener('change', (e) => {
            window.exportOnlyModified = (e.target.value === 'modified');
            console.log(`Export mode changed: Modified Only = ${window.exportOnlyModified}`);

            // Re-sync memory loading state so the next zip starts fresh with correct files
            assetsLoadedIntoMemory = false;
        });
    }
